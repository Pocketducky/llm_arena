...
обнулялись». Проверяет три уровня защиты, ВСЕ без обращения к Ollama:
  ...
  1. ollama_client.extract_json / _salvage_json_object — восстановление
...
Запуск:  python test_json_robustness.py
"""
...
def test_extract_json_recovery() -> None:
    ...
    truncated = ('{"A1": {"pass": true, "comment": "верно"}, '
                 ...
                 '"A3": {"pass": tr')
    r = ollama_client.extract_json(truncated)
...
def test_salvage_multiple_fields() -> None:
    ...
    salv = ollama_client._salvage_json_object(raw)
...
def test_score_round1_isolation() -> None:
    ...
    def fake_score_block(panel, role, block, ctx):
        if block == "C":
            raise ollama_client.OllamaError("смоделированный сбой JSON в блоке C")
