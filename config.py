...
# --- Быстрые пилотные профили (qwen2.5, БЕЗ reasoning-накладных) ---
# Введены, когда выяснилось, что qwen3:8b на локальном железе не успевает
# крупные промпты R2 (cross-review) и R3 (агрегатор) в лимит запроса и
# систематически таймаутит. qwen2.5 — не «мыслящая» модель, поэтому весь бюджет
# генерации идёт на ответ, а не на скрытый <think>; большие промпты укладываются
# в таймаут. Используются для прогона «только LLM-судьи» на пациенте Ж1.
...
# ══════════════════════════════════════════════════════════════════
# Текущий датасет (data/summaries.xlsx) — НЕ «полные суммаризации ЭМК
# для лечащего врача», а целевые выжимки под конкретного адресата:
# во всех картах суммаризации явно озаглавлены «Данные для рентгенолога»/
# «для врача-рентгенолога» (подготовка к описанию КТ ОБП). Это законно
# узкая задача — модель ОБЯЗАНА опустить сведения других профилей
# (офтальмология, ЛОР и т.п.), это не пропуск, а корректная фильтрация.
#
# gate.evaluate_gate() и промпты судей (judge._scope_description) знают
# про этот сценарий под ключом "radiologist": при scope="radiologist"
# (а) сырая сверка чисел/предикатов идёт не по ВСЕМУ объёму источника
#     (это дало бы системный ложный 100% reject — см. gate._self_check
#     и комментарий к evaluate_gate), а recall сущностей считается
#     против ОТФИЛЬТРОВАННОГО под задачу подмножества (filter_relevant_entities);
# (б) судьи получают корректное описание задачи вместо общего шаблона
#     «полное резюме по 5 разделам» — и не штрафуют за намеренные пропуски.
#
# Если датасет сменится на другой тип задачи — поменяйте константу здесь и при
# необходимости добавьте новый сценарий в gate._RELEVANCE_FILTER_PROMPTS
# и judge._SCOPE_DESCRIPTIONS.
DATASET_SCOPE: str | None = "radiologist"
