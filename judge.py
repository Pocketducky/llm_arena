...
def _ground_block_e(report: objective_layer.ObjectiveComparisonReport,
    ...
    """Заземление блока E (безопасность) — РЕАЛЬНЫЕ расхождения, без шума сжатия.

    Раньше сюда шёл весь hard_findings(). Измерено на eval_patient_zh1: у всех
    17 кандидатов 16 находок совпадали побайтово, и все были вида «сущность не
    найдена», то есть законные пропуски при сжатии 3.4:1. Специфичная для
    кандидата находка была лишь в 3 строках из 17 — сигнал/шум ≈ 1:17. При этом
    промпт требует «каждое ОБЯЗАТЕЛЬНО разбери: опасность (->E1) или пропуск
    (->E2)», то есть судье выдавалось 17 одинаковых готовых аргументов за
    E2.pass=false. Заземление, одинаковое для эталона и для антонимической
    инверсии, не может различать кандидатов — оно может только смещать всех.

    Компрессия не скрывается: она подаётся отдельной строкой и честно названа,
    чтобы судья мог учесть полноту, не принимая пропуски за расхождения.
    """
    real = report.hard_findings("real")
    n_compression = report.n_compression()

    if real:
        hard = "\n".join(f"  • {f}" for f in real[:30])
    else:
        hard = ("  (автоматическая сверка РЕАЛЬНЫХ расхождений — искажённых чисел, "
                "единиц измерения, инверсий полярности, введённой причинности, "
                "ложных интерпретаций — не нашла ничего; проверь текст "
                "содержательно на предмет более тонких искажений смысла)")
    if n_compression:
        hard += (f"\n  [справочно, НЕ расхождение] сущностей источника не вошло в "
                 f"суммаризацию: {n_compression}. Для сжатой выжимки это ОЖИДАЕМО и "
                 f"само по себе не является ни ошибкой, ни основанием для E1/E2 — "
                 f"оценивай полноту по существу задачи, а не по этому числу.")
    ...
    gate_summary = (f"{decision.status} — " + (", ".join(decision.reason_codes()) or "причин нет")
                    ...
                    + "\n  (вердикт шлюза — вспомогательный сигнал, он НЕ предрешает "
                      "твою оценку и не отменяет самостоятельный разбор текста)")
